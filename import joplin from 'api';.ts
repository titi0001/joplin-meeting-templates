...
// Function to copy simple template to clipboard
...
function generateTeamSelectionHTML(teams: TeamsConfig): string {
  let html = `
<div style="padding: 20px; font-family: Arial, sans-serif; max-height: 600px; overflow-y: auto;">
  <h2 style="margin-top: 0;">Multi-Team Meeting Setup</h2>
  ...
  Object.keys(teams).forEach((teamName, teamIndex) => {
    ...
    <div style="margin-bottom: 15px; border: 1px solid #ddd; padding: 10px; border-radius: 5px;">
      <div style="margin-bottom: 8px;">
        <label style="font-weight: bold; font-size: 14px; display: flex; align-items: center;">
          <input type="checkbox" id="team_${teamIndex}" name="selectedTeams" value="${teamName}" 
                 onchange="toggleParticipants(${teamIndex})" 
      ...
      <div id="participants_${teamIndex}" style="margin-left: 20px; display: none;">
        <p style="margin: 5px 0; font-weight: bold; color: #666; font-size: 12px;">Select participants:</p>
`;

    teams[teamName].forEach((participant, participantIndex) => {
      html += `
        <label style="display: flex; align-items: center; margin: 3px 0; font-size: 13px;">
